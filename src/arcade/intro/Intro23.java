...
public class Intro23 {
	 ...
	 * For
	 * 
	 * image = [[1, 1, 1], [1, 7, 1], [1, 1, 1]] the output should be boxBlur(image)
	 * = [[1]].
	 * 
	 ...
	 * should be
	 * 
	 * boxBlur(image) = [[5, 4], [4, 4]] There are four 3 × 3 squares in the input
	 ...
	 * 
	 * Guaranteed constraints: 3 ≤ image.length ≤ 100, 3 ≤ image[0].length ≤ 100, 0
	 * ≤ image[i][j] ≤ 255.
	 * 
	...
	public static void main(String[] args) {
		...
		int[][] image = {{1, 1, 1, 2, 4}, {1, 7, 1, 2, 4}, {1, 1, 1, 2, 4}, {1, 1, 1, 2, 4}, {1, 1, 1, 2, 4}};
		System.out.println("Blur image: " + boxBlur(image)); 

	}

	static int[][] boxBlur(int[][] image) {

		int qtdXLinhas = image.length - 2;
		int qtdXColunas = 0;
		int qtdXTotal = 0;
		
		if (image[0].length > 3) {
			qtdXColunas = image[0].length - 2;
			qtdXTotal = qtdXLinhas * qtdXColunas;
		} else {
			qtdXTotal = qtdXLinhas;
		}
		
		int count = 0;
		int[][] blurImagem = new int[image.length][image[0].length];
		while (count < qtdXTotal) {
			int countXLinhas = 0;
			while (countXLinhas < qtdXLinhas) {
		...
		System.out.println("Total: " + qtdXTotal);
		return blurImagem;
	}
