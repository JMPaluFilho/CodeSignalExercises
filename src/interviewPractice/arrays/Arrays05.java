...
public class Arrays05 {
     ...
     *
     * It is guaranteed that solution only contains entries for the letters present in crypt and that different letters
    ...
    public static void main(String[] args) {
        ...
        System.out.println(solution(crypt2, solution2));
    }

    static boolean solution(String[] crypt, char[][] solution) {
        Map<Character, Character> charValues = new HashMap<>(solution.length);
        StringBuilder x = new StringBuilder();
        StringBuilder y = new StringBuilder();
        StringBuilder z = new StringBuilder();

        for (char[] values : solution) {
            charValues.put(values[0], values[1]);
        }

        for (int i = 0; i < crypt.length; i++) {
            char[] letters = crypt[i].toCharArray();

            for (char letter : letters) {
                if (i == 0) {
                    x.append(charValues.get(letter));
                }

                if (i == 1) {
                    y.append(charValues.get(letter));
                }

                if (i == 2) {
                    z.append(charValues.get(letter));
                }
            }
        }

        if (x.charAt(0) == '0' && x.length() > 1
                || y.charAt(0) == '0' && y.length() > 1
                || z.charAt(0) == '0' && z.length() > 1) {
            return false;
        }

        BigDecimal sum = new BigDecimal(x.toString()).add(new BigDecimal(y.toString()));

        return sum.equals(new BigDecimal(z.toString()));
    }
}
